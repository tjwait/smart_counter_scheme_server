...
static LOGSET *getlogset() {
	...
	(_tcsrchr(path, _T('\\')))[1] = NULL;
	//getcwd(path, sizeof(path));
	//strcat(path, "/log.conf");
	//以下代码为wchar 转 char
	int iSize;
	char  char_path[256] = { 0 };
	iSize = WideCharToMultiByte(CP_ACP, 0, path, -1, NULL, 0, NULL, NULL); //iSize =wcslen(pwsUnicode)+1=6
	//char_path = (char*)malloc(iSize * sizeof(char) + 50); //不需要 pszMultiByte = (char*)malloc(iSize*sizeof(char)+1);
	WideCharToMultiByte(CP_ACP, 0, path, -1, char_path, iSize, NULL, NULL);
	strcat(char_path, "log.conf");//将反斜杠更改为windows样式
	if (access(char_path, F_OK) == 0) {
		if (ReadConfig(char_path) != 0) {//正常情况下是等于0的，因此不会进入该if语句内
