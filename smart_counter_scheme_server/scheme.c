...
*	功能：server端生成一个解决方案
*	参数：
*	说明：此函数是服务器端生成解决方案的主函数，引入方案唯一名称和要解算的items链表数据，链表数据是由客户端发送来的
*			items链表中的计价类型必须为2，这个条件控制方案上位机处理
*/
int Server_Scheme_Create(char * scheme_id, char * scheme_name, struct Items * item_p/*items链表*/, int error_value/*误差值*/, int save/*是否将方案保存*/)
{
	...
	{
		test_new(&scheme/*这个链表为最终结果*/, itp,
			buy_num /*单次消费者能够购买多少种商品*/, 0/*该值为循环递归调用深度，深度值等于商品种类,首次调用该函数时永远为0*/,
			&number/*这个值为递归调用时记录之前所有层拿取商品数量的，为一个工具类数值，函数调用完毕后不使用*/ , stock_kind);
...
int test_new(struct scheme * scheme_list/*这个链表为最终结果*/, struct Items * items_list,
	...
	struct number * number/*这个值为递归调用时记录之前所有层拿取商品数量的，为一个工具类数值，函数调用完毕后不使用*/,int stock_kind)
{
	int num_buf = 0;
					...
					for (int k = 0; k <= depth; k++)//depth编号是items_list的每一个元素编号，而且无需减1，因此此循环k的值要一直取到depth
					{
						if (number[k].num > 0)//在本类商品中拿取了货物
						{
							...
							if (list_node_p == NULL)
							{
								...
								list_node_p->ind_price = CharNum_To_Double(number[k].addr->ind_price);//商品单价
